...
import time
class Astar():
    def __init__(self, initState, objState, preDirection=None, depth=0,prestate=None,stateList=None,Manhattan=None):
    ...
    def getFvalue2(self, objState):
        list=self.matixToList(self.state)
        wValue = 0
        for i in range(0,9):
            for j in range(0,9):
                if(self.stateList[j]==list[i]):
                    wValue=wValue+self.Manhattan[j][i]
        return wValue + self.depth
        ...
        for i in self.direction:
            if i == "up":
                if self.blanlRow == 0:
                    ...
                    k = Astar(tempMartix, self.objState, "up", (self.depth) + 1,stateList= self.stateList,Manhattan=self.Manhattan)
                    k.preState = self
            ...
            if i == "down":
                if self.blanlRow == 2:
                    ...
                    tempMartix[self.blanlRow][self.blankColumn] = j
                    tempMartix[self.blanlRow + 1][self.blankColumn] = ' '
                    k=Astar(tempMartix, self.objState, "down", (self.depth) + 1,stateList= self.stateList,Manhattan=self.Manhattan)
                    k.preState = self
                    subModeCollection.append(k)
                    self.subModeSum = self.subModeSum + 1
                    k.preState = self
            if i == "left":
                if self.blankColumn == 0:
                    ...
                    k=Astar(tempMartix, self.objState, "left", (self.depth) + 1,stateList= self.stateList,Manhattan=self.Manhattan)
                    k.preState = self
                    subModeCollection.append(k)
                    self.subModeSum = self.subModeSum + 1
            ...
            if i == "right":
                if self.blankColumn == 2:
                    ...
                    tempMartix[self.blanlRow][self.blankColumn] = j
                    tempMartix[self.blanlRow][self.blankColumn + 1] = ' '
                    k=Astar(tempMartix, self.objState, "right", (self.depth) + 1,stateList= self.stateList,Manhattan=self.Manhattan)
                    k.preState = self
                    subModeCollection.append(k)
                    self.subModeSum = self.subModeSum + 1
    ...
    def getTheSuitableOneAStar(self):
        i = self.open[0].Fvalue
        suitableObject = self.open[0]
        for j in self.open:
            if (j.Fvalue  < i ):
    ...
    def solveproblemByAstar(self, objectState):
        start = time.clock()
        ...
        while 1:
            mostSuitaleState = self.getTheSuitableOneAStar()
